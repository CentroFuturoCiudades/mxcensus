...
| `denue.py` | `load_denue(state=N, release=…, harmonize=)` — fetches a DENUE release/state geoparquet, optionally **harmonizes** it to the latest schema (g10) via per-group rename/`per_ocu` maps, Pandera-validates. Multi-temporal economic-units directory (24 releases 2010–2025). |
...
- `denue_schema_map.yaml` – DENUE column-fingerprint → schema group (g01..g11), group→columns, and `latest` (harmonization target); `variables_denue_<gNN>.yaml` – per-group DENUE variable dictionaries (generated by `scripts/build_denue.py`)
...
python scripts/build_denue.py --variables           # regenerate variables_denue_<gNN>.yaml
...
onto the latest 42-column schema (rename + `per_ocu` canonicalization) so releases are
longitudinally comparable, and Pandera-validates. `harmonize=False` returns the raw schema.
Source URL quirks (state-15 multipart from 2018, the 2013-Jul/Oct shared filename, state-18
2015 date) live in `data/_denue_catalog.py`; the cache key is release-qualified to avoid
collisions.

The harmonization spec (`_RENAME`, `_PER_OCU`) is hard-coded in `denue.py`, **pinned to
`g10`'s mnemonic column names** — `_latest_schema` reads `latest` dynamically from the map,
but the rename targets do not. If a future release introduces a new majority schema that
becomes `latest`, revisit those dicts. The report's §7 lists each group's all-null columns
(e.g. g04's empty `entidad`/`municipio` name fields in 2012 states 12/14 — faithful to
source, codes-only); a column being all-null is data quality, whereas a *stale* rename map
emits a `warnings.warn` at load time.
