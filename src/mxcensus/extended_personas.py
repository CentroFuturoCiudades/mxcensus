...
def dhsersal_create_dummies(df):
    """Expand dual health-service fields (DHSERSAL1/2) into one-hot indicator columns plus PUB/AFIL aggregates."""
    dhsersal_or_cats = [
        "DHSERSAL_IMSS",
        "DHSERSAL_ISSSTE",
        "DHSERSAL_ISSSTE_E",
...
def med_traslado_esc_create_dummies(df):
    """Combine up to three school-commute transport fields into binary dummy columns."""
...
def med_traslado_trab_create_dummies(df):
    """Combine up to three work-commute transport fields into binary dummy columns."""
...
def dis_create_agg_cols(df):
    """Derive DIS_CON (condition) and DIS_LIMI (limitation) aggregate flags from the six disability columns."""
...
def get_educ_col(df):
    """Map the NIVACAD + ESCOLARI combination to a coarser ordered EDUC category."""
...
def preprocessor(df):
    """Apply all derived-column and category-mapping transformations to the raw personas DataFrame."""
    variables = variables_personas()
    df = df.copy()
    ...
    return pa.DataFrameSchema(
        {
            ...
            "ENT_PAIS_NAC_CAT": pa.Column(
                pd.CategoricalDtype(
                    ["EstaEnt", "OtraEnt", "OtroPais", "No especificado"]
                ),
            ),
            ...
            "QDIALECT_INALI": pa.Column(
                pd.CategoricalDtype(
                    list(set(variables["QDIALECT_INALI"]["Categorías"].values()))
                ),
            ),
            "HESPANOL": pa.Column(
            ...
            "MUN_ASI": pa.Column(
                pd.CategoricalDtype(
                    list(expand_cat_map(variables["MUN_ASI"]["Categorías"]).values())
                ),
            ),
            ...
            "MED_TRASLADO_ESC1": pa.Column(
                pd.CategoricalDtype(
                    variables["MED_TRASLADO_ESC1"]["Categorías"].values()
                ),
            ),
            ...
            "ESCOLARI": pa.Column(
                pd.CategoricalDtype(
                    list(expand_cat_map(variables["ESCOLARI"]["Categorías"]).values()),
                    ordered=True,
                ),
            ),
            "EDUC": pa.Column(
            ...
            "ESCOACUM": pa.Column(
                pd.CategoricalDtype(
                    list(expand_cat_map(variables["ESCOACUM"]["Categorías"]).values())
                )
            ),
            "ENT_PAIS_RES_5A": pa.Column(
                pd.CategoricalDtype(
                    list(
                        expand_cat_map(
                            variables["ENT_PAIS_RES_5A"]["Categorías"]
                        ).values()
                    )
                )
            ),
            ...
            "MUN_RES_5A": pa.Column(
                pd.CategoricalDtype(
                    list(expand_cat_map(variables["MUN_RES_5A"]["Categorías"]).values())
                )
            ),
            ...
            "OCUPACION_C_COARSE": pa.Column(
                pd.CategoricalDtype(
                    variables["OCUPACION_C_COARSE"]["Categorías"].values()
                )
            ),
            "SITTRA": pa.Column(
            ...
            "ACTIVIDADES_C_COARSE": pa.Column(
                pd.CategoricalDtype(
                    sorted(
                        list(
                            set(
                                variables["ACTIVIDADES_C_COARSE"]["Categorías"].values()
                            )
                        )
                    )
                )
            ...
            "MUN_TRAB": pa.Column(
                pd.CategoricalDtype(
                    list(expand_cat_map(variables["MUN_TRAB"]["Categorías"]).values())
                ),
            ),
            "ENT_PAIS_TRAB": pa.Column(
                pd.CategoricalDtype(
                    list(
                        expand_cat_map(
                            variables["ENT_PAIS_TRAB"]["Categorías"]
                        ).values()
                    )
                ),
            ),
            ...
            "MED_TRASLADO_TRAB1": pa.Column(
                pd.CategoricalDtype(
                    variables["MED_TRASLADO_TRAB1"]["Categorías"].values()
                ),
            ),
            ...
            "HIJOS_NAC_VIVOS": pa.Column(
                pd.CategoricalDtype(
                    list(
                        set(
                            expand_cat_map(
                                variables["HIJOS_NAC_VIVOS"]["Categorías"]
                            ).values()
                        )
                    )
                ),
            ...
            "HIJOS_FALLECIDOS": pa.Column(
                pd.CategoricalDtype(
                    list(
                        expand_cat_map(
                            variables["HIJOS_FALLECIDOS"]["Categorías"]
                        ).values()
                    )
                ),
            ),
            "HIJOS_SOBREVIV": pa.Column(
                pd.CategoricalDtype(
                    list(
                        expand_cat_map(
                            variables["HIJOS_SOBREVIV"]["Categorías"]
                        ).values()
                    )
                ),
            ),
            ...
            "SOBREVIVENCIA": pa.Column(
                pd.CategoricalDtype(
                    list(
                        expand_cat_map(
                            variables["SOBREVIVENCIA"]["Categorías"]
                        ).values()
                    )
                ),
            ),
            "IDENT_HIJO": pa.Column(
                pd.CategoricalDtype(
                    list(expand_cat_map(variables["IDENT_HIJO"]["Categorías"]).values())
                ),
            ),
            ...
            "EDAD_MORIR_TD": pa.Column(
                pd.CategoricalDtype(
                    list(
                        expand_cat_map(
                            variables["EDAD_MORIR_TD"]["Categorías"]
                        ).values()
                    )
                ),
            ),
...
def load_extended_personas(
    ...
    *,
    ...
    Explicit path (parquet from the mirror or your own file)::
        ...
        load_extended_personas(Path("personas_14.parquet"))
    ...
    if state is not None:
        ...
        survey_path = Path(POOCH.fetch(f"personas_{code}.parquet"))
