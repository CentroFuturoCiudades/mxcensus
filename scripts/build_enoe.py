...
``docs/enoe/HANDOFF.md`` for the full-mirror build + upload runbook.
...
Metadata modes (from parquet already on disk, no download):
    ...
    uv run python scripts/build_enoe.py --update-registry # → append enoe_* hashes to registry.txt
"""
...
def main() -> None:
    ...
    parser.add_argument("--update-registry", action="store_true",
                        help="Skip downloading; append enoe_* hashes to registry.txt (preserving prior)")
    ...
    if args.update_registry:
        written = sorted(args.output.glob("enoe_*.parquet"))
